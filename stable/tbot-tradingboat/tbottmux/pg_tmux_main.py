...
class TmuxTbot:
    ...
    def _create_window_per_cmd(self, command: str, window_name: str):
        ...
        logger.trace(f"new window_name={window_name}")
        for window in self.session_default.windows:
            logger.trace(f"window={window.window_name}")
            if window.window_name == window_name:
                logger.error(f"Error - boof command= {command}, window={window.window_name} already exists")
                return
    ...
    def tb_start(self, command: str, window_name: str):
        ...
        """
        logger.trace(f"new tb_start: command={command}, window_name={window_name}")
    ...
    def tb_stop(self, cleanup_command=None):
        ...
        if cleanup_command:
            logger.info(f"Running cleanup command: {cleanup_command}")
        else:
...
def get_cmdline():
    ...
    tbot = TmuxTbot()
    if action == "start":
        logger.info(f"action={action}, command={command}, window_name={window_name}")
        if command and window_name:
            logger.info(f"Starting TBOT with command: {command} in window: {window_name}")
