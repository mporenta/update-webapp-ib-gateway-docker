...
def start(
    ...
    def print_gui_info():
        if open_gui:
            logger.info('GUI is set to [OPEN] - it will be served at the root path.')
            logger.info(f'\n\tView GUI dashboard here: http://{host}:{port}\n')
        else:
            logger.info('GUI is set to [CLOSED] - it will be served at the path /?guiKey=<unique_key>')
            ...
            logger.info(f'To learn more about GUI modes, visit: {gui_modes_url}')
