...
def safe_float(x):
    try:
        ...
        return None
async def ta_calcs(symbol: str, daily_aggs: list[dict], minute_aggs: list[dict], polyData=False) -> dict:
    try:
        data= {}
        ...
        logger.info(f"daily_aggs len {len(daily_aggs)} is polyData? {polyData}")
...
async def gpt_data(contract: Contract, ib: IB= None) -> dict:
    """
    ...
    try:
        ...
        analysis  = await ta_calcs(contract.symbol, df_d, df_m, polyData=False)
        return analysis
