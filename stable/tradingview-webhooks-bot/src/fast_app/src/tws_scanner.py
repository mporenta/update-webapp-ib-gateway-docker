...
async def wh_horizons():
    ...
    events = await ib.getWshEventDataAsync(datWsha)
    logger.info(events)
    ib.disconnect()
...
async def get_earnings_dates(symbol: str, limit=10):
    """
    Get recent and upcoming earnings dates for a contract.
    
    Args:
        contract: Contract object to get earnings dates for
        limit: Maximum number of earnings dates to return
        
    Returns:
        JSON string containing earnings date events
    """
    try:

        raw_contract = Stock(symbol, 'SMART', 'USD')
        # First qualify the contract to get the conId
        qualified_contracts = await ib.qualifyContractsAsync(raw_contract)
        if not qualified_contracts:
            raise ValueError(f"Could not qualify contract: {raw_contract}")
        contract= qualified_contracts[0]
        conId = qualified_contracts[0].conId
        
        # Ensure we have WSH metadata (required before requesting event data)
  
        await ib.getWshMetaDataAsync()
    
    # Create WshEventData object with filter for earnings dates
        data = WshEventData(
            filter = f'''{{
                "country": "All",
                "watchlist": ["{conId}"],
        ...
        if earnings_dates_found:
            logger.info(f"Boof is a winner: {prev_day}, Day of Week: {prev_day.weekday()} is weekend: {is_weekend} and trading_day_tomorrow: {trading_day_tomorrow} saturday: {saturday} sunday: {sunday}")

        else:
            logger.info("No earnings dates found")
        logger.info(f"Previous trading day: {prev_day}, Day of Week: {prev_day.weekday()} is weekend: {is_weekend} and trading_day_tomorrow: {trading_day_tomorrow} saturday: {saturday} sunday: {sunday}")
