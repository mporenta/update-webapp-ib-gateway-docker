...
aggregated_bars = defaultdict(lambda: defaultdict(pd.DataFrame))

def convert_barsize_to_resample_rule(barSizeSetting):
    """
    Convert IB bar size setting to pandas resample rule.
    
    Parameters:
    - barSizeSetting (str): Bar size like "1 min", "5 min", "1 hour", etc.
    
    Returns:
    - str: Pandas resample rule
    """
    # Parse the bar size setting
    parts = barSizeSetting.split()
    if len(parts) != 2:
        raise ValueError(f"Invalid bar size setting: {barSizeSetting}")
...
async def build_ohlc_bars(symbol, barSizeSetting):
    """
    Build OHLC bars for a given symbol and bar size from stored 5-second bars.
    ...
    Returns:
    - pd.DataFrame: DataFrame with OHLC data for the requested bar size
    """
    try:
        # Convert barSizeSetting to pandas resample rule
        resample_rule = convert_barsize_to_resample_rule(barSizeSetting)
        
        # Get 5-second bars for the symbol
        bars_5s = list(confirmed_5s_bar[symbol])
        if not bars_5s:
            logger.warning(f"No 5-second bars available for {symbol}")
            return pd.DataFrame()
        
        # Convert to DataFrame
        df_5s = pd.DataFrame(bars_5s)
        
        # Ensure there's data
        if df_5s.empty:
            logger.warning(f"Empty DataFrame for {symbol} 5-second bars")
            return pd.DataFrame()
        ...
        if not isinstance(df_5s.index, pd.DatetimeIndex):
            df_5s.index = pd.to_datetime(df_5s.index)
        
        # Sort by time
        df_5s = df_5s.sort_index()
        
        # Resample to the requested bar size
        df_resampled = df_5s.resample(resample_rule).agg({
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
        ...
        vStop, uptrend, nine_ema = await calculate_indicators_for_bars(df_resampled)
        
        # Store in the global structure
        aggregated_bars[symbol][barSizeSetting] = df_resampled
        
        logger.info(f"Built {barSizeSetting} OHLC bars for {symbol}: {len(df_resampled)} bars")
        
        return df_resampled
    
    except Exception as e:
        logger.error(f"Error building OHLC bars for {symbol} with {barSizeSetting}: {e}")
        return pd.DataFrame()

async def calculate_indicators_for_bars(df, atrlen=20, atrfactor=1.5):
    """
    Calculate indicators (vStop, ema9, uptrend) for a DataFrame of OHLC bars.
    Similar to the existing vol_stop function but works with any bar size.
    
    Parameters:
    - df (pd.DataFrame): DataFrame with OHLC data
    - atrlen (int): ATR length for volatility stop calculation
    - atrfactor (float): ATR factor for volatility stop calculation
    
    Returns:
    - Tuple: (vStop Series, uptrend Series, ema9 Series)
    """
    try:
        logger.info("Calculating Indicators for bars...")
        
        # Calculate average of high and low prices
        avg_hl = (df['high'] + df['low']) / 2
        
        # Calculate ATR using pandas_ta
        atr_pandas_ta = atr(df['high'], df['low'], df['close'], atrlen)
        
        # Calculate 9-period EMA
        nine_ema = ema(df['close'], length=9).tolist() 
        
        # Multiply ATR by factor
        atrM = atr_pandas_ta * atrfactor
        atrM = atrM.fillna(0)  # Fill NaN values with 0
        
        # Use avg_hl for close in volatility stop calculation
        close = avg_hl
        ...
        logger.error(f"Error calculating indicators for bars: {e}")
        return None, None, None

async def get_ohlc_bars(symbol, barSizeSetting, lookback=None):
    """
    Get OHLC bars for a given symbol and bar size. If they don't exist, build them.
    
    Parameters:
    - symbol (str): The ticker symbol
    - barSizeSetting (str): Bar size like "1 min", "5 min", "1 hour", etc.
    - lookback (int, optional): Number of bars to return. If None, return all bars.
    
    Returns:
    - pd.DataFrame: DataFrame with OHLC data for the requested bar size
    """
    # Check if we have the bars already
    if symbol in aggregated_bars and barSizeSetting in aggregated_bars[symbol]:
        bars = aggregated_bars[symbol][barSizeSetting]
    else:
        # Build the bars
        bars = await build_ohlc_bars(symbol, barSizeSetting)
    
    # Return the requested number of bars
    if lookback is not None and not bars.empty:
...
# Modified on_bar_update function that updates aggregated bars
async def on_bar_update(bars, has_new_bar):
    ...
    # Update all active aggregated bar sizes for this symbol
    for barSizeSetting in aggregated_bars[symbol].keys():
        await build_ohlc_bars(symbol, barSizeSetting)
# Contract handler
...
async def get_ohlc():
    try:
        ...
        # Check if the symbol exists in the subscribed contracts
        if symbol not in subscribed_contracts:
            # If not, create a contract and subscribe
            contract = Stock(symbol, 'SMART', 'USD')
            qualified_contract = await qualify_contract(contract)
            await add_contract(qualified_contract, barSizeSetting)
        
        # Get the OHLC bars
        bars = await get_ohlc_bars(symbol, barSizeSetting, lookback)
        
        if bars.empty:
            return JSONResponse(
                content={"status": "error", "message": f"No OHLC data available for {symbol} with {barSizeSetting}"},
                status_code=404
            )
        
        # Convert DataFrame to JSON-compatible format
        bars_json = bars.reset_index().to_dict(orient='records')
        
        # Format datetime objects to strings
        for bar in bars_json:
            if 'time' in bar and isinstance(bar['time'], datetime):
                bar['time'] = bar['time'].isoformat()
        
        return JSONResponse(
            content={"status": "success", "symbol": symbol, "barSizeSetting": barSizeSetting, "bars": bars_json}
        )
    
    except Exception as e:
        logger.error(f"Error getting OHLC data for {symbol} with {barSizeSetting}: {e}")
        return JSONResponse(
