...
class LogConfig:
    ...
    def setup(self):
        ...
        # Add file logging
        self.logger.add(
            sink=os.path.join(self.root_dir, "app_tbot.log"),
            level=self.log_level,
            format=self.log_format,
            rotation="10 MB",
            retention="1 week",
        ...
        self.logger.add(
            sink=os.path.join(self.root_dir, "error_tbot.log"),
            level="ERROR",
        ...
        if self.log_level == "DEBUG":
            self.logger.add(
                sink=os.path.join(self.root_dir, "debug_tbot.log"),
                level="DEBUG",
                format=self.debug_format,
                rotation="10 MB",
                retention="3 days",
...
if __name__ == "__main__":
    ...
    self.logger.info(f"Current Positions:{log_config.format_positions(positions)}")
    
    account_values = {...}  # Your account values
    self.logger.info(f"Account Values:{log_config.format_account_values(account_values)}")
    """    